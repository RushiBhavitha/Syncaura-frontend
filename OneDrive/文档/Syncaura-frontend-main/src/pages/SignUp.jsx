...
const SignUp = () => {
  ...
  const { initiateGithubLogin, isLoading: isGithubLoading } = useGithubLogin();
                ...
                {socialProviders.map((p) => (
                  <motion.button
                    ...
                    style={{
                      ...
                      borderRadius: "0px",
                      opacity: p.isLoading ? 0.6 : 1,
                    }}
                    className="w-11 h-11 border flex items-center justify-center social-btn disabled:cursor-not-allowed"
                    aria-label={`Sign up with ${p.alt}`}
                    id={`signup-${p.id}-btn`}
                  >
                    {p.isLoading ? (
                      <Loader className="w-5 h-5 animate-spin" style={{ color: t.inputText }} />
                    ) : (
                      <img
                        src={p.icon}
                        alt={p.alt}
